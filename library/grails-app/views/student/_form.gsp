...
<div class="fieldcontain ${hasErrors(bean: studentInstance, field: 'course', 'error')} required">
	...
	<g:select id="course" name="course.id" from="${com.Course.list()}" optionKey="id" required="" value="${studentInstance?.course?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: studentInstance, field: 'books', 'error')} ">
	...
	<g:select name="books" from="${com.Book.list()}" multiple="multiple" optionKey="id" size="5" value="${studentInstance?.books*.id}" class="many-to-many"/>
